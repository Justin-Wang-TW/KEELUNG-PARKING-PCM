...
import { Asset, AssetStatus, StationCode, User, UserRole, AssetCheckRecord, AssetCheckStatus, AssetCheckBatch, AssetCheckItemResult } from '../types';
...
const AssetManagement: React.FC<AssetManagementProps> = ({ currentUser, assets, checkRecords, checkBatches = [], onDelete, onSubmitCheck, onSaveBatch, onSaveBatchAssets, onEdit }) => {
  ...
  const availableStations = useMemo(() => {
    ...
    if (currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.MANAGER_3D || currentUser.assignedStation === 'ALL') {
      return STATIONS;
    }
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter(asset => {
      ...
      const hasPermission = currentUser?.role === UserRole.ADMIN || 
                            currentUser?.role === UserRole.MANAGER_3D || 
                            userStations.includes('ALL') || 
                            userStations.includes(asset.stationCode);
  ...
  const assetsByStation = useMemo(() => {
    ...
    filteredAssets.forEach(asset => {
      if (!groups[asset.stationName]) {
        groups[asset.stationName] = [];
      }
      groups[asset.stationName].push(asset);
    });
    return groups;
  }, [filteredAssets]);
  ...
  }, [checkRecords, filterStation, searchTerm]);
    ...
    if (checkBatches && checkBatches.length > 0) {
      ...
      filteredRecords.forEach(record => {
        dataToExport.push({
          ...
          '期別': '-',
          '場站': STATIONS.find(s => s.code === record.stationCode)?.name || record.stationCode,
          ...
          <div className="p-6 overflow-y-auto flex-1">
            <form id="batchCheckForm" onSubmit={handleSubmitBatchCheck} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">場站</label>
                  <select
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-gray-800">財產紀錄與管理</h2>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">場站</th>
                    ...
                    {filteredAssets.length > 0 ? (
                      filteredAssets.map((asset) => (
                        <tr key={asset.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{asset.stationName}</td>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">場站</th>
                      ...
                      filteredBatches.map((batch) => (
                        <React.Fragment key={batch.id}>
                          <tr className="hover:bg-gray-50 cursor-pointer" onClick={() => setExpandedBatchId(expandedBatchId === batch.id ? null : batch.id)}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{batch.stationName}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-7xl w-full overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              {/* Left Panel: Form */}
              <div className="w-full md:w-5/12 p-6 border-r border-gray-200 overflow-y-auto bg-white">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">場站 (Station)</label>
                      <select
